...
def build_engine(cfg, datamanager, model, optimizer, scheduler):
    if cfg.data.type == 'image':
        ...
        if cfg.loss.name == 'engine_as_net':
            engine = torchreid.engine.ImageAsNetEngine(
                datamanager,
                model,
                optimizer,
                margin=cfg.loss.triplet.margin,
        ...
        if cfg.loss.name == 'engine_as_net':
            engine = torchreid.engine.VideoAsNetEngine(
                datamanager,
                model,
                optimizer,
                margin=cfg.loss.triplet.margin,
...
def reset_config(cfg, args):
    ...
    cfg.data.save_dir = cfg.data.save_dir+'/'+cfg.data.exp_name
...
def main():
    ...
    if osp.exists(cfg.data.save_dir):
        print('!!!The name of experiment already exists!!!')
        exit(1)
